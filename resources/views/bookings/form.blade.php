...
<div class="alert alert-warning">
    ...
    <a href="{{ route('maskapai.index') }}">Master Maskapai</a> dan
    <a href="{{ route('wilayah.index') }}">Master Wilayah</a>.
</div>
@endif

<form action="{{ $booking ? route('bookings.update', $booking) : route('bookings.store') }}" method="POST">
    @csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="issued_date" class="form-control"
                        value="{{ old('issued_date', isset($booking) ? $booking->issued_date->format('Y-m-d') : date('Y-m-d')) }}" required>
                </div>
                <div class="col-md-2">
                    <label class="form-label">Mata Uang</label>
                    <input type="text" name="currency" class="form-control" value="{{ old('currency', $booking->currency ?? 'IDR') }}">
                </div>
                <div class="col-md-2">
    ...
    <div class="text-end mb-5">
        <a href="{{ route('bookings.index') }}" class="btn btn-outline-secondary btn-lg">Batal</a>
...
<script>
    ...
    function buildOptions(list, selectedId) {
        return list.map(item => `<option value="${item.id}" ${String(item.id) === String(selectedId) ? 'selected' : ''}>${item.label}</option>`).join('');
    }
    ...
    function passengerTemplate(i, p = {}) {
        const titleOptions = ['Mr.', 'Mrs.', 'Ms.'].map(t => `<option value="${t}" ${p.title === t ? 'selected' : ''}>${t}</option>`).join('');
        const typeOptions = ['Adult', 'Child', 'Infant'].map(t => `<option value="${t}" ${p.type === t ? 'selected' : ''}>${t}</option>`).join('');
    ...
    function addPassenger(data = {}) {
        document.getElementById('passengerWrapper').insertAdjacentHTML('beforeend', passengerTemplate(passengerIndex, data));
        passengerIndex++;
    }
